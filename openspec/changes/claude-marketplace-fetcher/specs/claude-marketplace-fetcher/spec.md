...
The `Plugin` value object SHALL hold a spec, commit SHA, and list of relative file paths. It does NOT hold file content — files stay on disk in the plugin cache.

#### Scenario: Plugin created from fetch result
- **GIVEN** a fetcher downloads a plugin with 3 files to cache
- **WHEN** a `Plugin` is constructed with spec, commit SHA, and file paths
...
### Requirement: PluginFetcher protocol downloads plugin directories to cache
The `PluginFetcher` protocol SHALL define `fetch(spec, cache_dir) -> Plugin` which downloads all files in a plugin directory to `cache_dir` and returns a `Plugin` manifest.

#### Scenario: Fetcher downloads plugin to cache
- **WHEN** `fetch(spec, cache_dir)` is called
- **THEN** all plugin files are written to `cache_dir/{registry}/{plugin}/{commit_sha}/`
- **AND** a `Plugin` manifest is returned with the commit SHA and file list
