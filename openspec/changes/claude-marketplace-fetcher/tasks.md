...
- [ ] 5.4 Update `fetch(spec, cache_dir) -> Plugin`: return `Plugin(spec, files, source_dir=prompts_dir)` instead of `Prompt`
...
- [ ] 6.9 Implement `fetch(spec, cache_dir) -> Plugin`: orchestrate marketplace lookup → SHA check → directory listing → download → Plugin construction
...
- [ ] 8.3 Single loop: for each spec, resolve fetcher by registry name, call `fetch()`, create LockEntry
- [ ] 8.4 For local plugins: content_hash from file hashing, commit_sha=None
