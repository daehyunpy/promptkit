...
- **THEN** `skills/foo.md` is removed from `.cursor/`, `skills/bar.md` is removed from `.claude/`, `skills/baz/SKILL.md` is removed from `.agents/`, all manifest files are deleted, and empty parent directories are pruned
...
- **THEN** the use case maps platform name `codex` to output directory `.agents`
- **AND** removes `skills/my-skill/SKILL.md` from `.agents/`
