...
- [ ] 1.1 Create `source/promptkit/infra/builders/manifest.py` with `MANIFEST_FILENAME = ".promptkit-managed"`, `MANIFEST_HEADER = "# Generated by promptkit — do not edit"`
- [ ] 1.2 Implement `read_manifest(fs, output_dir) -> list[str]` — reads `.promptkit-managed`, returns list of relative paths (skipping comment lines), returns empty list if file doesn't exist
- [ ] 1.3 Implement `write_manifest(fs, output_dir, paths: list[str])` — writes sorted relative paths with header comment
- [ ] 1.4 Implement `cleanup_managed_files(fs, output_dir, manifest_paths: list[str])` — removes each listed file if it exists, then removes empty parent directories within output_dir
...
- [ ] 2.1 Replace `self._fs.remove_directory(output_dir)` in `ClaudeBuilder.build()` with: read manifest → cleanup managed files → write new artifacts → write new manifest
...
- [ ] 3.1 Replace `self._fs.remove_directory(output_dir)` in `CursorBuilder.build()` with: read manifest → cleanup managed files → write new artifacts → write new manifest
...
- [ ] 4.1 Update the `.gitignore` template in the init scaffolding to remove the commented-out `.cursor/` and `.claude/` lines (or add a comment explaining they should be committed)
